...
async def start_chat_with_llm(message: Dict, x_session_id: str = Header(None, alias="X-Session-ID")):
    ...
    if title == "":
        title =  "Blood test interpretation chat"
    title = title[:50] + "..."
    chat_response = {
        "chat_id": chat_id,
        "title": title,
...
@router.get("/files/{chat_id}/{message_id}.csv")
async def get_chat_file(chat_id: str, message_id: str):
    """
    Serve a csv from the chat history.
    ...
    try:
        ...
        file_path = os.path.join(
            chat_manager.files_dir,
            chat_id,
            f"{message_id}.csv"
        )
        
        # Verify the path exists and is within the files directory
        file_path = Path(file_path).resolve()
        files_dir = Path(chat_manager.files_dir).resolve()
        
        # Security check: ensure the requested file is within the files directory
