...
from api.utils.llm_rag_utils import chat_sessions, create_chat_session, generate_chat_response, rebuild_chat_session
...
# @router.get("/csv/{chat_id}/{message_id}.csv")
# async def get_chat_csv(chat_id: str, message_id: str):
#     """
#     Serve a CSV file from the chat history.
...
#     Returns:
#         FileResponse: The CSV file with appropriate content type
#     """
#     try:
#         # Construct the CSV path
#         csv_path = os.path.join(
#             chat_manager.csv_dir,
#             chat_id,
#             f"{message_id}.csv"
#         )
        
#         # Verify the path exists and is within the CSV directory
#         csv_path = Path(csv_path).resolve()
#         csv_dir = Path(chat_manager.csv_dir).resolve()
        
#         # Security check: ensure the requested file is within the CSV directory
#         if not str(csv_path).startswith(str(csv_dir)):
#             raise HTTPException(
#                 status_code=403,
...
#         content_type, _ = mimetypes.guess_type(str(csv_path))
#         if not content_type:
